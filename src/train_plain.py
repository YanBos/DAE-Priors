...
parser.update_utils()
net = models.PlainNet(utils.NSZ, batch_norm=True).model
ds, ds_cb, vds = dataset.prepare_dataset(gd=False)
adam = tf.keras.optimizers.Adam(learning_rate=utils.LR)
net.compile(optimizer=adam, loss=tf.keras.losses.MSE)
cbs = [] if not os.path.exists('../tf-logs') else [utils.ES, utils.TB, cb.Image(ds_cb)]
net.fit(ds,
        epochs=utils.EPOCHS,
